...
if __name__ == '__main__':
    usage = 'gen_write.py <filename> <filesize> <req_count> [[<req_min_size>,]<req_max_size>]'
    ...
    print 'Genearte to %s, filesize: %s Bytes. Write %s times.' %(
                            sys.argv[1], filesize, req_count)
    ...
    with open(sys.argv[1], 'w') as f:
        f.write('#Generated by gen_write.py\n')
        f.write('filesize:%s req_count:%s\n' %(filesize, req_count))
        f.write('# req_size_range:%s\n' %(req_size_range,))
        while req_count > 0:
            count += 1
            size = rand_req_size(filesize, req_size_range)
            offset = rand_req_offset(filesize, size)
            f.write('%-6s  offset: %-12s bytes: %-12s\n' %(count, offset, size))
            req_count -= 1
            bytes_sum += size
        f.write('#bytes_sum: %s Bytes' %bytes_sum)
